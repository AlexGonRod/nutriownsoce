...
import { API_ENDPOINT, API_BASIC_INFO, API_ADVANCE_INFO } from "../src/lib/const"
...
test('fetch default data', async ({ page, request }) => {
  ...
  const response = await request.get(`${API_ENDPOINT}3017624010701${API_BASIC_INFO}`)
  const response2 = await request.get(`${API_ENDPOINT}3017624010701${API_ADVANCE_INFO}`)
  expect(response.ok()).toBeTruthy();
  expect(response2.ok()).toBeTruthy();

  // await page.goto('/');
  const title = page.getByText(nutella.Title.toUpperCase())
